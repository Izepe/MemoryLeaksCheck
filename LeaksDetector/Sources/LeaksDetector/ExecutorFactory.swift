...
enum ParameterKeys {
  ...
  static let diagnosticsFolderPath: String = "diagnosticsFolderPath"
}
...
enum ExecutorFactory {
  static func createExecutor(
    ...
    case .file:
      guard let diagnosticsPath = parameters[ParameterKeys.diagnosticsFolderPath] as? String else {
        log(message: "❌ Used of type `maestro` will need to specify --maestro-flow-path", color: .red)
        return nil
      }
      return FileExecutor(memgraphsFolderPath: diagnosticsPath)
    }
  }
}
